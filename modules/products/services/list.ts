...
export const list = async (
  ...
  try {
    ...
    let query = supabase
      .from("product")
      .select("*, product_category(*), supplierInfo(*), Action(User(*))");

    if (filters.q) {
      // Use OR logic for multiple field search
      query = query.or(
        `product_category.name.ilike.%${filters.q}%,supplierInfo.name.ilike.%${filters.q}%,supplier.ilike.%${filters.q}%,name.ilike.%${filters.q}%,description.ilike.%${filters.q}%`,
      );
    }
