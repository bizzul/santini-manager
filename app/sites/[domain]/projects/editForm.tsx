  ...
  useEffect(() => {
    ...
    const getClients = async () => {
      try {
        ...
        if (!d.ok) throw new Error("Failed to fetch clients");
    ...
    const getProducts = async () => {
      try {
        ...
        if (!d.ok) throw new Error("Failed to fetch products");
        const data = await d.json();
    ...
    const getHistory = async () => {
      try {
        const response = await fetch(`/api/tasks/${resource.id}/history`, {
          headers: getHeaders(),
        });
        if (response.ok) {
          const data = await response.json();
    ...
    const initializeForm = async () => {
      ...
      form.setValue("ora_fine", resource.ora_fine ?? null);
