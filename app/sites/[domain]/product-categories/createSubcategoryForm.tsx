...
import { createSellSubcategory } from "./actions/create-subcategory.action";
...
const CreateSubcategoryForm = ({
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    try {
      const response = await createSellSubcategory(
        { name: values.name, description: values.description, categoryId },
        domain,
      );
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  {...field}
                  disabled={isSubmitting || Boolean(createdSubcategory)}
                  placeholder="es. Porte per interni, finiture standard"
                />
              </FormControl>
              <FormMessage />
            </FormItem>
