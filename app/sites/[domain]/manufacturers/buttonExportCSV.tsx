...
const CSV_COLUMNS = [
  ...
  { key: "manufacturer_category.name", header: "CATEGORIA" },
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!manufacturers || manufacturers.length === 0) {
        toast({
          variant: "destructive",
          description: "Nessun produttore da esportare",
        });
        return;
      }
      ...
      // Create data rows
      const rows = manufacturers.map((manufacturer: any) => {
        return CSV_COLUMNS.map((col) => {
          // Handle nested keys like "manufacturer_category.name"
          if (col.key.includes(".")) {
            const keys = col.key.split(".");
            let value = manufacturer;
            for (const k of keys) {
              value = value?.[k];
            }
            return escapeCSVValue(value);
          }
          return escapeCSVValue(manufacturer[col.key]);
        });
      });
      ...
      link.href = url;
      link.download = `produttori_export_${formatLocalDate(new Date())}.csv`;
      ...
      toast({
        description: `Esportati ${manufacturers.length} produttori con successo!`,
      });
    } catch (error: any) {
      toast({
        variant: "destructive",
