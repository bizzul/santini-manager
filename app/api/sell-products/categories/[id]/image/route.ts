...
const log = logger.scope("SellProductCategoryImage");
...
export async function POST(
  ...
  try {
    ...
    const fileName = `${siteId}/sell-categories/${categoryId}/${uuidv4()}.webp`;
    ...
    if (uploadError) {
      log.error("Sell category upload error:", uploadError);
      return NextResponse.json(
    ...
    if (category.image_url) {
      const oldPath = extractCategoryImagePath(category.image_url);
      if (oldPath) {
    ...
    log.error("Sell category image upload error:", error);
    const message =
...
export async function DELETE(
  ...
  try {
    ...
    if (category.image_url) {
      const imagePath = extractCategoryImagePath(category.image_url);
      if (imagePath) {
    ...
    log.error("Sell category image delete error:", error);
    const message =
      error instanceof Error ? error.message : "Internal server error";
    return NextResponse.json({ error: message }, { status: 500 });
  }
