...
const log = logger.scope("SupplierCategories");
...
export async function GET(req: NextRequest) {
    try {
        ...
        if (!siteId) {
            log.warn("Supplier Categories API called without siteId");
        ...
        log.error("Supplier Categories API error:", error);
        return NextResponse.json(
            { error: "Error fetching supplier categories" },
            { status: 500 },
        );
    }
}
...
export async function POST(req: NextRequest) {
    try {
        ...
        log.error("Supplier Categories POST error:", error);
        return NextResponse.json(
            { error: "Error creating supplier category" },
            { status: 500 },
        );
    }
}
