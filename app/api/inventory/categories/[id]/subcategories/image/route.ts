...
const log = logger.scope("InventorySubcategoryImage");
...
export async function POST(
  ...
  try {
    ...
    if (categoryError) {
      return NextResponse.json(
        { error: `Errore database: ${categoryError.message}` },
        { status: 500 },
      );
    }
    ...
    const fileName = `${siteId}/subcategories/${categoryId}/${uuidv4()}.webp`;
    ...
    if (uploadError) {
      log.error("Subcategory upload error:", uploadError);
      return NextResponse.json(
    ...
    log.error("Subcategory image upload error:", error);
    const message =
...
export async function DELETE(
  ...
  try {
    ...
    log.error("Subcategory image delete error:", error);
    const message =
      error instanceof Error ? error.message : "Internal server error";
    return NextResponse.json({ error: message }, { status: 500 });
  }
