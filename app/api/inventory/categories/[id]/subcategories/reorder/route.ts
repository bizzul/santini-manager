...
const log = logger.scope("InventorySubcategoryReorder");
...
export async function PATCH(
  ...
  try {
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      if (lookupError) {
        log.error("Subcategory lookup error:", lookupError);
        return NextResponse.json(
          {
            error:
      ...
      if (existing) {
        ...
        if (updateError) {
          log.error("Subcategory reorder update error:", updateError);
          return NextResponse.json(
            {
              error:
        ...
        if (insertError) {
          log.error("Subcategory reorder insert error:", insertError);
          return NextResponse.json(
            {
              error:
    ...
    log.error("Subcategory reorder error:", error);
    const message =
