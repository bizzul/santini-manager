...
import { createItem } from "@/app/sites/[domain]/manufacturer-categories/actions/create-item.action";
...
type CategorySelectorProps = {
  categories: Manufacturer_category[];
  value?: number | string;
  onValueChange: (value: string) => void;
  disabled?: boolean;
  domain: string;
  onCategoryCreated?: (category: Manufacturer_category) => void;
};

export const ManufacturerCategorySelector: React.FC<CategorySelectorProps> = ({
  ...
  const [categories, setCategories] =
    useState<Manufacturer_category[]>(initialCategories);
  ...
  const refreshCategories = async () => {
    try {
      ...
      console.error("Error refreshing manufacturer categories:", error);
      toast({
        title: "⚠️ Attenzione",
        description: "Impossibile aggiornare la lista delle categorie",
        variant: "destructive",
    ...
    <div className="flex gap-2">
      ...
      >
        <SelectTrigger className="flex-1">
          <SelectValue placeholder="Seleziona categoria produttore..." />
        </SelectTrigger>
        <SelectContent>
          {categories.map((cat: Manufacturer_category) => (
      ...
      <Button
        ...
        disabled={disabled}
        title="Crea nuova categoria produttore"
      >
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Nuova categoria produttore</DialogTitle>
            <DialogDescription>
...
export default ManufacturerCategorySelector;
