...
interface AvorWeeklyTrendChartProps {
  data: AvorDashboardStats["weeklyTrend"];
  columnNames: string[];
}

export default function AvorWeeklyTrendChart({
  data,
  columnNames,
}: AvorWeeklyTrendChartProps) {
  if (data.length === 0) {
    return (
  ...
  const chartOptions: ApexOptions = {
    ...
    tooltip: {
      ...
      intersect: false,
      y: {
        formatter: (value) => `${value} nuove pratiche`,
      },
    },
    legend: {
      position: "bottom",
