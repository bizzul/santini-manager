...
interface ResaleCategoryChartProps {
  data: ProductsDashboardStats["resale"]["productsByCategory"];
}

export default function ResaleCategoryChart({
  data,
}: ResaleCategoryChartProps) {
    ...
    return (
      <div className="backdrop-blur-xl bg-white/10 dark:bg-black/10 border border-white/20 dark:border-white/10 rounded-2xl shadow-xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-bold">Numero prodotti per categoria</h3>
        </div>
      ...
      events: {
        dataPointSelection: (event, chartContext, config) => {
          ...
          if (category?.categoryId) {
            window.location.href = `/sites/${domain}/products?categoryId=${category.categoryId}`;
          }
        },
      },
    },
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Distribuzione prodotti attivi per categoria
          </p>
        </div>
      </div>
