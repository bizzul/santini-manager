...
interface ProductionTrendChartProps {
  data: ProductsDashboardStats["production"]["elementsByWeek"];
}
...
export default function ProductionTrendChart({
    ...
    return (
      <div className="backdrop-blur-xl bg-white/10 dark:bg-black/10 border border-white/20 dark:border-white/10 rounded-2xl shadow-xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-bold">Elementi prodotti nel tempo</h3>
        </div>
  ...
  const chartSeries = [
    {
      name: "Elementi prodotti",
      data: data.map((d) => d.elements),
    },
  ];
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Andamento produzione pezzi per settimana
          </p>
        </div>
      </div>
